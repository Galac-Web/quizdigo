...
function saveQuizConfig(string $configDir, array $payload): void
{
    $builderLayoutsPath = $configDir . DIRECTORY_SEPARATOR . 'builder-layouts.json';
    $questionTypesPath = $configDir . DIRECTORY_SEPARATOR . 'question-types.json';
    ...
    $questionTypeId = trim((string)($questionTypeItem['id'] ?? ''));
    $questionTypeName = trim((string)($questionTypeItem['name'] ?? ''));
    if ($questionTypeId === '' || $questionTypeName === '') {
        respond(['success' => false, 'message' => 'Quizzul trebuie sa aiba ID si nume inainte de salvare.'], 422);
    }
    ...
    $questionTypes = readJsonFile($questionTypesPath);
    $questionTypes['questionTypes'] = is_array($questionTypes['questionTypes'] ?? null) ? $questionTypes['questionTypes'] : [];
    upsertById($questionTypes['questionTypes'], $questionTypeItem);

    writeJsonFile($builderLayoutsPath, $layouts);
    writeJsonFile($questionTypesPath, $questionTypes);
}

function saveThemeConfig(string $configDir, array $payload): void
{
    $builderThemesPath = $configDir . DIRECTORY_SEPARATOR . 'builder-themes.json';
    $builderCorePath = $configDir . DIRECTORY_SEPARATOR . 'builder-core.json';
    ...
    if (empty($themeItem['id'])) {
        respond(['success' => false, 'message' => 'Lipseste ID-ul temei pentru salvare.'], 422);
    }

    $themes = readJsonFile($builderThemesPath);
    $themes['themes'] = is_array($themes['themes'] ?? null) ? $themes['themes'] : [];
    $themes['themeImageLibrary'] = is_array($themes['themeImageLibrary'] ?? null) ? $themes['themeImageLibrary'] : [];
    upsertById($themes['themes'], $themeItem);
    mergeLibraryItems($themes['themeImageLibrary'], (array)($themeItem['imageLibrary'] ?? []));
    ...
    if (!empty($themeItem['id'])) {
        $core['common']['defaults']['theme']['themeId'] = (string)$themeItem['id'];
    }
    if (!empty($themeItem['url'])) {
        $core['common']['defaults']['theme']['backgroundUrl'] = (string)$themeItem['url'];
    }
...
try {
    ...
    if ($action === 'save_quizz' && $method === 'POST') {
        $payload = parseJsonBody();
        saveQuizConfig($configDir, $payload);
        respond([
            'success' => true,
            'message' => 'Quizzul a fost salvat in fisierele proiectului.',
        ]);
    }

    if ($action === 'save_theme' && $method === 'POST') {
        $payload = parseJsonBody();
        saveThemeConfig($configDir, $payload);
        respond([
            'success' => true,
            'message' => 'Tema a fost salvata in fisierul proiectului.',
        ]);
    }
